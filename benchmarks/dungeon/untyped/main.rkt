...
(require/configurable-contract "utils.rkt" reset! random-from random-between)

(provide/configurable-contract
 (N ((max exact-nonnegative-integer?) (types exact-nonnegative-integer?)))
 (wall-cache ((max (hash/c array-coord? boolean?)) (types hash?)))
 (free-cache ((max (hash/c array-coord? boolean?)) (types hash?)))
 (animate-generation? ((max boolean?) (types boolean?)))
 (ITERS ((max exact-nonnegative-integer?) (types exact-nonnegative-integer?)))
 (dungeon-height
  ((max exact-nonnegative-integer?) (types exact-nonnegative-integer?)))
 (dungeon-width
  ((max exact-nonnegative-integer?) (types exact-nonnegative-integer?)))
 (try-add-rectangle
  ((max
     ...
     (result
      ...
      (or-#f/c
       (room-with/c
        ...
        any/c)))))
   (types
    (-> grid? array-coord? index? index? direction? (or-#f/c any-room?)))))
 (commit-room
  ((max
    (->i
 ...
 (hash-clear! ((max any/c) (types any/c)))
 (smooth-walls ((max (-> grid? grid?)) (types (-> grid? grid?))))
 (smooth-single-wall
  ((max (-> grid? array-coord? void?)) (types (-> grid? array-coord? void?))))
 (LOOPS ((max exact-nonnegative-integer?) (types exact-nonnegative-integer?)))
 (main ((max any/c) (types any/c))))
...
(define animate-generation? #f)

(define ITERS 10)

(define dungeon-height 18)
...
(define (counts-as-free? grid pos)
  (cond
