...
(require (only-in "message-queue.rkt" enqueue-message! message-queue))
...
(define/contract
 door%
 (configurable-ctc
  (max (make-cell%/c-with self #\* not-equal?))
  (types cell%/c))
 (class cell%
   (inspect #f)
   (inherit-field occupant)
...
(module+
 test
 (require rackunit)
 (define player% (class object% (define/public (show) #\@) (super-new)))
 ...
 (check-equal? (send vertical-wall-c show) #\║)
