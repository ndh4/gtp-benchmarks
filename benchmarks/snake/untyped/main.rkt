...
(require (only-in "handlers.rkt" game-over? handle-key))
...
(provide replay DATA LOOPS main)
 ...
 (configurable-ctc
  (max
   (->
    world-type?
    (listof
     (or/c (list/c 'on-key string?) (list/c 'on-tick) (list/c 'stop-when)))
    void?))
...
(define/contract LOOPS (configurable-ctc (max (=/c 1)) (types natural?)) 1)
