  ...
  (flat-named-contract
   ...
   (λ (p)
       (match p
         ...
         [_ #f]))
   (λ (fuel)
     (define int-generator (contract-random-generate/choose integer? (sub1 fuel)))
...
(define/ctc-helper
  ...
  (flat-named-contract
   ...
   (λ (s)
     (match s
       [(snake (? snake-dir?)
               (? nonempty-snake-segs?)) #t]
       [_ #f]))
   (λ (fuel)
     (define snake-dir?-generator (contract-random-generate/choose snake-dir? (sub1 fuel)))
     (define snake-segs?-generator (contract-random-generate/choose nonempty-snake-segs? (sub1 fuel)))
...
(define/ctc-helper
  ...
  (flat-named-contract
   (string->symbol (format "(world/c ~a ~a)" (contract-name snake/c) (contract-name food/c)))
   (λ (x)
       (match x
           [(world (? snake/c) (? food/c)) #t]
           [_ #f]))
   (λ (fuel)
     (define snake-generator (contract-random-generate/choose snake/c (sub1 fuel)))
     (define food-generator (contract-random-generate/choose food/c (sub1 fuel)))
