...
(require (only-in "command.rkt" command% CMD*))
...
(provide assert defn-command forth-eval* forth-eval forth-tokenize de-nest)
    ...
    (result
     (match
      ...
      ((cons (or ': 'define) (cons w defn*-any))
       (cons/c (cons/c command%? (equal?/c E)) (equal?/c S)))
      (_ #f)))))
  (types command%?))
 (new
  command%
             ...
             (if (equal? v (list w))
               (let-values (((e+ s+)
                             (for/fold
                              ((e E) (s S))
                              ((d (in-list defn*)))
                              (if e (forth-eval e s (list d)) (values e s)))))
                 (if e+ (cons e+ s+) e+))
               #f)))))
       (cons (cons cmd E) S))
      (_ #f))))))
...
(define/contract
 ...
 (configurable-ctc
  (max
   (->i
    ((E env?) (S stack?) (token* token*?))
    (values
     (env-result (E) (or/c (or/c #f (equal?/c E)) env?))
     (stack-result (S) (or/c (equal?/c S) stack?)))))
  ...
  (max
   (->i
    ...
    (result
     (str)
     (and/c
      (listof (or/c number? symbol?))
      (lambda (result) (= (length result) (length (string-split str))))))))
  (types (-> string? token*?)))
 (for/list
...
(define/ctc-helper
 (nested-singleton-list? v)
 (and (list? v) (= (length v) 1) (list? (first v))))
...
#;(module+
 ...
 (define E1 CMD*)
 (define eval/stack
   (lambda (token*) (let-values (((e s) (forth-eval E1 S1 token*))) s)))
