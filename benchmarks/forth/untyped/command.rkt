...
(provide/configurable-contract
 (assert ((max any/c) (types any/c)))
 (command% ((max command%/c) (types command%/c)))
 (singleton-list?
  ((max
    (->i
     ((x any/c))
   ...
   (types (-> any/c boolean?))))
 (binop-command% ((max binop-command%/c) (types binop-command%/c)))
 (CMD*
  ((max
    (and/c
     env?
     (list/c
      (command%?-with-exec
       (args E S v)
       (result
        (if (or (eof-object? v) (is-or-starts-with? exit? v)) 'EXIT #f)))
      (command%?-with-exec
       (args E S v)
      ...
      (command%?-with-exec
       ...
       (result
        (if (and (is-or-starts-with? (curry equal? 'swap) v)
                 ((list-with-min-size/c 2) S))
          (equal?/c
           (cons E (cons (second S) (cons (first S) (rest (rest S))))))
          #f)))
      (command%?-with-exec
       (args E S v)
       (result
