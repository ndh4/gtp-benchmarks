...
(define/ctc-helper (listof/any-depth/c ctc)
    (define name (format "(listof/any-depth/c ~a)"
                         (contract-name ctc)))
    (make-contract
     ...
     (λ (fuel)
       (define base-generator
         (contract-random-generate/choose ctc fuel))
       (case fuel
         [(0) base-generator]
         [else
          (define ladc-generator
            (contract-random-generate/choose
             (listof/any-depth/c ctc) (sub1 fuel)))
          (thunk
           (if (zero? (random 2))
               (base-generator)
               (let loop ()
