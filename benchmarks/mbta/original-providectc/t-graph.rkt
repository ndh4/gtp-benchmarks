...
(provide/configurable-contract
 ...
 [read-t-line-from-file ([max (->i ([lf color?])
                                   [result (lf)
                                           (λ (res)
                                             (andmap (λ (pair)
                                                       (and (line? (first pair))
 ...
 [lines->hash ([max (->i ([lines line-spec-list/c])
                         [result (lines)
...
(define/ctc-helper (line-spec-list? p)
  (and (list? p)
       (pair? p)
...
(define/ctc-helper (line-spec-list-station-first? p allowed-lines)
  (and (list? p)
       (pair? p)
       (station? (first p))
       (line-spec-list-remainder? (rest p) allowed-lines)))

(define/ctc-helper (line-spec-list-remainder? p allowed-lines)
  (and (list? p)
       (or (null? p)
           (and (station? (first p)) (line-spec-list-remainder? (rest p) allowed-lines))
           (let ([regexp-res (regexp-match #px"--* (.*)" (first p))])
             (and regexp-res
                  (andmap (λ (e) (member e allowed-lines))
                          (string-split (second regexp-res))))))))

(define/ctc-helper line-spec-list/c
  (flat-named-contract
   'line-spec-list/c
   line-spec-list?
   (λ (fuel)
     ...
     (define lines-generator (contract-random-generate/choose (and/c pair? (listof line?)) fuel))
     (define station-generator (contract-random-generate/choose station? fuel))
     (define (make-starter lines)
       (string-append-immutable (list->string (dashes-generator))
                      " "
                      (string-join lines " ")))
     (thunk
        ...
        (cond
          ...
          [else
           ...
           (if (member generated-station disallowed-stations)
               ...
               (if (zero? (random 2))
                   (cons (make-starter (take (shuffle allowed-lines) (add1 (random (length allowed-lines)))))
                         (cons generated-station
                               (generate-remainder (cons generated-station disallowed-stations) (sub1 fuel))))
                   (cons generated-station
                         (generate-remainder (cons generated-station
                                                   disallowed-stations) (sub1 fuel)))))]))
      (define generated-station (station-generator))
      (cons (make-starter allowed-lines)
            (cons generated-station
