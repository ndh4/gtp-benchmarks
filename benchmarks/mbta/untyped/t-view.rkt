    ...
    (result
     ...
     (λ (out-lst)
       (and (list? out-lst)
            ...
            (andmap
             (lambda (l)
               (<= (count-strs-in-lst out-lst) (count-strs-in-lst l)))
             inp-lst))))))
  (types (-> (listof (listof any/c)) (listof any/c))))
...
(define/contract
 CURRENT-LOCATION-0
 (configurable-ctc (max "no such station: ~a") (types string?))
 "no such station: ~a")

(define/contract
 DESTINATION
 (configurable-ctc (max "disambiguate your destination: ~a") (types string?))
 "disambiguate your destination: ~a")

(define/contract
 DESTINATION-0
 (configurable-ctc (max "no such destination: ~a") (types string?))
...
(define/contract
 DISABLED
 (configurable-ctc (max "clarify station to be disabled: ~a") (types string?))
 "clarify station to be disabled: ~a")

(define/contract
 ENABLED
 (configurable-ctc (max "clarify station to be enabled: ~a") (types string?))
 "clarify station to be enabled: ~a")

(define/contract
 DISABLED-0
 (configurable-ctc (max "no such station to disable: ~a") (types string?))
 "no such station to disable: ~a")

(define/contract
 ENABLED-0
 (configurable-ctc (max "no such station to enable: ~a") (types string?))
 "no such station to enable: ~a")

(define/contract
 ENSURE
 (configurable-ctc (max "---ensure you are on ~a") (types string?))
 "---ensure you are on ~a")

(define/contract
 SWITCH
 (configurable-ctc (max "---switch from ~a to ~a") (types string?))
 "---switch from ~a to ~a")

(define/ctc-helper
 (expected-stations-matching s)
 (filter (λ (other-s) (string-contains? other-s s)) expected-stations))

(define/ctc-helper
 (num-expected-stations-matching s)
...
(define/ctc-helper
 ...
 (class/c
  ...
  (find
   (->i
    ((self any/c) (from string?) (to string?))
    (result (self from to) (λ (res) (correct-find-result? self from to res)))))
  (field (mbta-subways (instanceof/c mbta%/c)) (disabled (listof station?)))))
...
(define/ctc-helper
 ...
 (and (string? res)
      (or (string-contains? res from) (string-contains? res to))
      (cond
       ((and (= from-count 1)
             (= to-count 1)
             ...
             any-disabled?)
        #t)
       ((or (not (= from-count 1)) (not (= to-count 1)))
       ...
       (else
        ...
        (define path-parts
          (map
           (λ (line)
             (if (regexp-match? "---|impossible|tap your heels" line)
               line
...
(define/ctc-helper
 (valid-path? parts underlying-graph)
 (local-require "../base/my-graph.rkt")
 (for/and
  ((prev-station (in-list parts)) (next-station (in-list (rest parts))))
  (match*
   (prev-station next-station)
   (((list p pline) (list n nline)) (has-edge? underlying-graph p n))
   (((list p pline) (? string?)) #t)
   (((regexp #rx"---switch from ([^ ]+) to ([^ ]+)" (list _ from to))
     (list n nline))
    (not (equal? from to)))
   (((? string?) (list p pline)) #t)
   ((_ _) #f))))

(define/ctc-helper
 (sufficient-path? path-parts underlying-graph from to)
 (local-require "../base/my-graph.rkt")
 (define underlying-graph-path (fewest-vertices-path underlying-graph from to))
 (define no-path?
   (and (= (length path-parts) 1)
 ...
 (and (or no-path?
      ...
      (implies no-path? (not underlying-graph-path))
      (implies underlying-graph-path (not no-path?))))
...
(define/ctc-helper
 ...
 (class/c
  ...
  (field (mbta-subways (is-a?/c mbta%)) (disabled list?))))

(define/contract
 manage%
 (configurable-ctc (max manage-c/max-ctc) (types manage-c/types-ctc))
 (class object%
   (super-new)
   ...
   (define/private
    ...
    (cons
     start
     (let loop ((pred-lines pred-lines0)
                (pred-string pred-string0)
                (path (rest path0)))
       (cond
        ((empty? path) '())
        (else
...
(provide manage-c/max-ctc manage-c/types-ctc)
...
#;(module+
 ...
 (check-equal?
  '("Stony Brook Station" "Brookline Hills Station" "Riverway Station")
  (get-field disabled manage1))
